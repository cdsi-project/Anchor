...
pf_check_package_manager() {
    local status value
    local label="${CDSI_PACKAGE_BACKEND:-package manager}"
    ...
    pf_print_result "$label" "$value" "$status"
    pf_update_status "$status"
}
...
pf_check_database_service() {
    local status value
    if check_command mysql \
       || check_command mariadb \
       || cdsi_service_installed "${CDSI_DB_SERVICE}" \
       || cdsi_service_installed mariadb; then
        value="Installed"
        status="$PF_WARN"
    else
        value="Not Installed"
...
pf_check_selinux() {
    ...
    pf_print_result "SELinux" "$mode" "$status"
    pf_update_status "$status"
}
...
pf_check_firewalld() {
    ...
    if cdsi_service_active firewalld; then
        ...
        value="Inactive (check cloud firewall)"
        status="$PF_WARN"
    fi
    pf_print_result "firewalld" "$value" "$status"
    pf_update_status "$status"
}
